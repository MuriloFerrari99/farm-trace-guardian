-- First run the previous migration
INSERT INTO storage_areas (name, area_code, zone_type, capacity_kg, temperature_range_min, temperature_range_max, is_certified)
VALUES 
  ('Área de Recebimento', 'AR001', 'recebimento', 5000, 18, 25, true),
  ('Câmara Fria A', 'CFA001', 'resfriado', 10000, 2, 8, true),
  ('Câmara Fria B', 'CFB001', 'resfriado', 10000, 2, 8, true),
  ('Área de Expedição', 'AE001', 'expedicao', 3000, 15, 25, true);
...
SELECT 
  ...
  CASE 
    WHEN sa.zone_type = 'recebimento' THEN 10
    WHEN sa.zone_type = 'resfriado' THEN 20  
    WHEN sa.zone_type = 'expedicao' THEN 8
    ELSE 5
...
CROSS JOIN generate_series(1, 
  CASE 
    WHEN sa.zone_type = 'recebimento' THEN 15
    WHEN sa.zone_type = 'resfriado' THEN 25
    WHEN sa.zone_type = 'expedicao' THEN 10
    ELSE 10
  END
) as series(n)
