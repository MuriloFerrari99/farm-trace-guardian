...
-- 2. Fix CRM policies to use (select auth.uid()) instead of auth.uid()
...
-- 3. Fix CRM opportunities policies
...
-- 4. Fix CRM interactions policies
...
TO authenticated
USING (
  public.is_admin_or_supervisor((select auth.uid())) OR
  EXISTS (
...
-- 6. Update the helper functions to also use (select auth.uid()) pattern
...
-- 7. Update CRM policies to use the new parameter-less functions
DROP POLICY IF EXISTS "Optimized CRM contacts view" ON public.crm_contacts;
DROP POLICY IF EXISTS "Optimized CRM contacts update" ON public.crm_contacts;
DROP POLICY IF EXISTS "Optimized CRM opportunities view" ON public.crm_opportunities;
DROP POLICY IF EXISTS "Optimized CRM opportunities update" ON public.crm_opportunities;
